...
import { Box, Typography, Button, Slide, Link, LinearProgress } from "@mui/material";
...
const Dashboard = () => {
      ...
      },
      {
        ...
        renderCell(params) {
          return (
            ...
            >
              <LinearProgress variant="determinate" sx={{ height: 20, borderRadius: 50 }} value={params.value} />
              <Box
                sx={{
                  top: 0,
                  left: params.value < 10 ? params.value : undefined,
                  right: params.value > 10 ? `calc(100% - ${params.value}%)` : undefined,
                  bottom: 0,
                  position: "absolute",
                  display: "flex",
                  alignItems: "center",
  ...
  return (
    <>
      ...
      >
        <Typography sx={{ fontWeight: 600, fontSize: 20 }}>{pageHeaderText.OVERVIEW}</Typography>
        <Button
          sx={{
      ...
      >
        <Box sx={{ p: "16px 16px 4px 32px" }}>
          <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
            ...
            >
              {!isCollapse ? <KeyboardDoubleArrowDownIcon /> : <KeyboardDoubleArrowUpIcon />}
            </Button>
          </Box>
        </Box>
            ...
            >
              ...
              >
                <Typography sx={{ fontWeight: 600 }}>
                  ...
                  <Link onClick={() => navigate(PROJECT)} sx={{ color: "#0F69EF", ml: 0.5, cursor: "pointer" }}>
                    {dashboardText.VIEW_ALL}
                  </Link>
                </Typography>
                {/* TODO: This will be replace by Search Function in the future */}
                <SearchBar onChange={() => "hello"} />
              </Box>
